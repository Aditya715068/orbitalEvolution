...
class NotificationsDemo extends React.Component {
  ...
  render() {
 //   const { from } = this.props.location.state || { from: { pathname: '/ChangePassword' } };
    return (
      <ListGroup className={[s.listGroup, 'thin-scroll'].join(' ')}>
        <ListGroupItem className={s.listGroupItem}>
          ...
          <p className="m-0 overflow-hidden" onClick={()=>this.props.history.push("/ChangePassword") }>
           Change Password
          </p>
        </ListGroupItem>
        {/* <ListGroupItem className={s.listGroupItem}>
...
export default withRouter(NotificationsDemo);
