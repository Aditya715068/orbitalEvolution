...
const Static = (props) => {
  ...
  useEffect(() => {
    axios
      .get(
        `https://3uzeslc5fc.execute-api.us-east-1.amazonaws.com/dev/userList/${searchVal}`
      )
      .then((res) => {
  ...
  //   axios
  //     .get(
  //       `https://3uzeslc5fc.execute-api.us-east-1.amazonaws.com/dev/userList/${this.state.searchVal}`
  //     )
  //     .then((res) => {
  //       const users = res.data;
  //       this.setState({
  ...
  //       console.log(this.state.searchVal, this.state.userList);
  //     });
  // }
  ...
  const pageCount = Math.ceil(userData.length / 8);

  console.log("", currentItems, pageCount);

  // Invoke when user click to request another page.
  const handlePageClick = (event) => {
    const newOffset = (event.selected * 8) % userData.length;
    console.log(
  ...
  return (
    <div className={s.root}>
      <h2 className="page-title">
        Tables - <span className="fw-semi-bold">User Management</span>
      </h2>
      <Row>
        <Col lg={12} md={12} sm={12}>
          <Widget
            title={
              <h5>
                Users <span className="fw-semi-bold">Tables</span>
              </h5>
            }
            settings
            close
                ...
                <tbody>
                  {currentItems.map((row, index) => (
                    <tr key={row.id}>
                      ...
                      <td className="text-muted">{row.account_holder}</td>
